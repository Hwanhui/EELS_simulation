function varargout = Ebroadening_v2(varargin)

% Copyright 2017 H. Yun 
...
% Ebroadening_v2 MATLAB code for Ebroadening_v2.fig
%      Ebroadening_v2, by itself, creates a new Ebroadening_v2 or raises the existing
%      singleton*.
%
%      H = Ebroadening_v2 returns the handle to a new Ebroadening_v2 or the handle to
%      the existing singleton*.
%
%      Ebroadening_v2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in Ebroadening_v2.M with the given input arguments.
%
%      Ebroadening_v2('Property','Value',...) creates a new Ebroadening_v2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Ebroadening_v2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Ebroadening_v2_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @Ebroadening_v2_OpeningFcn, ...
                   'gui_OutputFcn',  @Ebroadening_v2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Ebroadening_v2 is made visible.
function Ebroadening_v2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Ebroadening_v2 (see VARARGIN)
I=imread('Logo.jpg'); 
axes(handles.Logo);
imshow(I, 'Parent', handles.Logo);
set(handles.Logo, 'visible','off');

% Choose default command line output for Ebroadening_v2
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Ebroadening_v2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);
end


% --- Outputs from this function are returned to the command line.
function varargout = Ebroadening_v2_OutputFcn(hObject, eventdata, handles) 
% varargout  cell array for returning output args (see VARARGOUT);
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

fprintf('\n Author: Hwanhui Yun (yunxx133@umn.edu). Updated: Aug 2017');
...
fprintf(' In case you dont want to include EELS instrumental energy resolution, enter 0 for EELS Energy Resolution (eV). \n')
...
function ERes_Callback(hObject, eventdata, handles)
% hObject    handle to ERes (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of ERes as text
%        str2double(get(hObject,'String')) returns contents of ERes as a double
end

% --- Executes during object creation, after setting all properties.
function ERes_CreateFcn(hObject, eventdata, handles)
% hObject    handle to ERes (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
set(Ebroadening_v2, 'HandleVisibility', 'off');
close all;
set(Ebroadening_v2, 'HandleVisibility', 'on');
...
E_res=get(handles.ERes,'String');E_res=str2num(cell2mat(E_res));
...
%Zero-padding
Res_01=zeros(1,N); Res_001=zeros(1,N);
Sim_02=zeros(1,11*N);
for i=5*N+1:6*N
...
if E_res==0
    ...
    for i=1:N
   x=(-N:N); E_res_2=E_res/dispE/2.35482;
   Ga=normpdf(x,0,E_res_2);
   Res_02=conv(Sim_02, Ga, 'same');
   Res_001(i)=Res_02(5*N+i);
   Sim_02(5*N+i)=Res_02(5*N+i);    
    end
    end
...
fourthline=sprintf(' - Final state E broadening was applied from %.2f to %.2f eV Energy range',E(1), E(N));xlswrite(name, cellstr(fourthline),1,'A5');
sixthline={'Energy (eV)','Intensity'};
xlswrite(name, Fin, 1, 'A7'); xlswrite(name, cellstr(firstline), 1, 'A1');xlswrite(name, cellstr(secondline), 1, 'A2'); xlswrite(name, sixthline, 1, 'A6:B6');

if shellv ==1 && z<15
    spr=sprintf(' - Initial state E broadening = %.2f eV',kz);
    xlswrite(name, cellstr(spr), 1, 'A4');
elseif shellv==2 && z<36 && z>12
    spr=sprintf(' - Initial state E broadening = %.2f eV',lz);
    xlswrite(name, cellstr(spr), 1, 'A4');
else
     xlswrite(name, cellstr(' - Initial state E broadening was not applied'), 1, 'A4');
end

if E_res > 0
    spr=sprintf(' - Instrumental E broadening = %.2f eV',E_res);
    xlswrite(name, cellstr(spr), 1, 'A3');
else
     xlswrite(name, cellstr(' - Instrumental E broadening was not applied'), 1, 'A3');
end
...
fprintf(fileID,' - Instrumental E broadening = %.2f eV\r\n',E_res);
if shellv ==1 && z<15
...
fprintf(fileID,' - "%s"file was used\r\n',raw);
if E_res > 0
    fprintf(fileID,' - Instrumental E broadening = %.2f eV',E_res);
else
    fprintf(fileID,' - Instrumental E broadening was not applied');
end
